...
class AbstractModel
{
    ...
    //$primaryKeyCheck we make it false if we sending the id but want to create not update
    //example at creating profile with the same id when we creating the user
    public function save($primaryKeyCheck = true)
    {
        if(false === $primaryKeyCheck) {
            return $this->create();
        }
		// if is edit so we already get the id from the url and we set the $primaryKey inside the model class 
    ...
    public static function getByPK($pk)
    {
        ...
        if ($stmt->execute() === true) {
			// if the class need construct values we add \PDO::FETCH_PROPS_LATE to avoid the error
			//so we check if there is construct method or not first
			// we fetch the data into our model class
            if(method_exists(get_called_class(), '__construct')) {
                $obj = $stmt->fetchAll(\PDO::FETCH_CLASS | \PDO::FETCH_PROPS_LATE, get_called_class(), array_keys(static::$tableSchema));
            } else {
                $obj = $stmt->fetchAll(\PDO::FETCH_CLASS, get_called_class());
            }
			//we use arrayshif because is just one row of table here , to not need to use for each
